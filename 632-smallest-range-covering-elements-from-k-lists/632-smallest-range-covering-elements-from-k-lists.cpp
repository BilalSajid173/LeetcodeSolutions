...
public:
    vector<int> smallestRange(vector<vector<int>>& nums) {
        ...
        for(int i = 0;i<nums.size();i++){
            pqmax.push({nums[i][0],{i,0}});
            pqmin.push({nums[i][0],{i,0}});
        }
        
        pair<int,int> ans;
        ans = {pqmin.top().first, pqmax.top().first};
        ...
        while(!pqmin.empty()){
            ...
            if((j+1) < nums[i].size()){
                ...
                pqmax.push({nums[i][j+1], {i,j+1}});
                int diff = pqmax.top().first - pqmin.top().first;
                if(diff < (ans.second - ans.first)){
                    ans.first = pqmin.top().first;
                    ans.second = pqmax.top().first;
                }
            } else{
                break;
            }
